...
import NextImage from "components/NextImage";
import CartProductPriceDisplay from "./CartProductPriceDisplay";
import ProductCardTopBar from "./ProductCardTopBar";
...
const ProductCard = (props) => {
  ...
  // Helper to get discount text for the top bar
  ...
  // Helper to get percentage discount for below image
  const getPercentDiscount = () => {
    if (
      Number.parseInt(item?.discount) > 0 &&
  ...
  // Helper to get MRP (original price)
  const getMrp = () => {
    if (item?.price && item?.discount && item?.discount_type === "percent") {
      ...
      return (parseFloat(item.price) + parseFloat(item.discount)).toFixed(2);
    }
    return null;
  };
  ...
  return (
    ...
    >
      {/* Top bar: discount and wishlist heart (no overlap) */}
      <ProductCardTopBar
        discountText={getDiscountText()}
        isWishlisted={isWishlisted}
        onAdd={addToWishlistHandler}
        onRemove={removeFromWishlistHandler}
      />
      {/* Product image (fully visible, not overlapped) */}
      <Box
        sx={{
          width: "100%",
          display: "flex",
      ...
      {/* Product description/details and price row */}
      <Box sx={{ p: 2, borderBottom: "1px solid #eee" }}>
        {/* Product name/description, always 2 lines visible */}
        <Typography
          variant={horizontalcard === "true" ? "subtitle2" : "body2"}
          sx={{
            color: (theme) => theme.palette.text.custom,
            fontSize: { xs: "13px", sm: "inherit" },
            overflow: "hidden",
        ...
        {/* Price, MRP, discount row below description */}
        <Stack direction="row" alignItems="center" spacing={1} mt={0.5}>
          ...
          {/* Percentage discount */}
          {getPercentDiscount() && (
            <Typography variant="body2" color="#FF6159" fontWeight={600}>
              {getPercentDiscount()}
            </Typography>
          )}
        </Stack>
      </Box>
      {/* Add to Cart button at the bottom, compact, no margin/padding */}
      <Box sx={{ p: 0, m: 0 }}>
        <AddWithIncrementDecrement
          onHover={state.isTransformed}
          addToCartHandler={addToCart}
