...
//   const [payableAmount, setPayableAmount] = useState(null);
//   const [address, setAddress] = useState(undefined);
//   const [showValidationErrors, setShowValidationErrors] = useState(false); // New state for validation display
//   const { couponInfo } = useSelector((state) => state.profileInfo);
//   const [paymentMethod, setPaymentMethod] = useState("");
//   const [numberOfDay, setDayNumber] = useState(getDayNumber(today));
//   const [offlinePayments, setOfflinePayments] = useState("");
...
//   const handlePlaceOrder = () => {
//     // Validate mandatory address fields for delivery orders
//     if (orderType !== "take_away") {
//       // Check address from multiple sources: address state, state from ScheduleReducer, and guestUserInfo
//       const streetValue =
//         address?.road ||
//         state?.streetNumber ||
//         (token ? null : guestUserInfo?.road);
//       const houseValue =
//         address?.house ||
//         state?.houseNumber ||
//         (token ? null : guestUserInfo?.house);
//       const floorValue =
//         address?.floor || state?.floor || (token ? null : guestUserInfo?.floor);

//       if (!streetValue?.trim() || !houseValue?.trim() || !floorValue?.trim()) {
//         // Show validation errors on the form
//         setShowValidationErrors(true);
//         toast.error(
//           "Please fill in Street Number, House Number, and Floor before placing the order.",
//           {
//             position: "bottom-right",
//           }
//         );
...
//   // Function to handle field changes and hide validation errors
