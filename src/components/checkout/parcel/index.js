...
const ParcelCheckout = () => {
  ...
  const formik = useFormik({
    initialValues: {
      password: "",
      confirm_password: "",
      senderRoad: parcelInfo?.senderRoad || "",
      senderHouse: parcelInfo?.senderHouse || "",
      senderFloor: parcelInfo?.senderFloor || "",
    },
    validationSchema: Yup.object({
      password: Yup.string()
        .required(t("Password is required"))
      ...
      confirm_password: Yup.string()
        .required(t("Confirm Password"))
        .oneOf([Yup.ref("password"), null], t("Passwords must match")),
      senderRoad: Yup.string().required(
        "Please fill the required field - Street/Road"
      ),
      senderHouse: Yup.string().required(
        "Please fill the required field - House number"
      ),
      senderFloor: Yup.string().required(
        "Please fill the required field - Floor"
      ),
    }),
  });

  // Add handlers to sync formik values with parcelInfo
  const senderRoadHandler = (value) => {
    formik.setFieldValue("senderRoad", value);
  };
  const senderHouseHandler = (value) => {
    formik.setFieldValue("senderHouse", value);
  };
  const senderFloorHandler = (value) => {
    formik.setFieldValue("senderFloor", value);
  };

  // Update formik values when parcelInfo changes
  useEffect(() => {
    if (parcelInfo?.senderRoad) {
      formik.setFieldValue("senderRoad", parcelInfo.senderRoad);
    }
    if (parcelInfo?.senderHouse) {
      formik.setFieldValue("senderHouse", parcelInfo.senderHouse);
    }
    if (parcelInfo?.senderFloor) {
      formik.setFieldValue("senderFloor", parcelInfo.senderFloor);
    }
  }, [parcelInfo]);

  // Trigger validation when formik values change
  useEffect(() => {
    if (
      formik.values.senderRoad ||
      formik.values.senderHouse ||
      formik.values.senderFloor
    ) {
      formik.validateForm();
    }
  }, [
  ...
  // Function to validate and show field errors
  const validateAndShowErrors = async () => {
  ...
  const orderMutationObject = {
    ...
    password: formik.values.password,
    is_guest: token ? 0 : 1,
    road: formik.values.senderRoad || parcelInfo?.senderRoad,
    house: formik.values.senderHouse || parcelInfo?.senderHouse,
    floor: formik.values.senderFloor || parcelInfo?.senderFloor,
  };
  ...
  const orderPlace = async () => {
    // Validate formik form first and show field errors
    const errors = await validateAndShowErrors();

    if (Object.keys(errors).length > 0) {
      // Show toast for any validation errors
      if (errors.senderRoad) {
        toast.error(errors.senderRoad, { position: "bottom-right" });
      }
      if (errors.senderHouse) {
        toast.error(errors.senderHouse, { position: "bottom-right" });
      }
      if (errors.senderFloor) {
        toast.error(errors.senderFloor, { position: "bottom-right" });
      }
      return;
    }

    // Check both formik values and parcelInfo values for validation
    const streetValue =
      formik.values.senderRoad?.trim() || parcelInfo?.senderRoad?.trim();
    const houseValue =
      formik.values.senderHouse?.trim() || parcelInfo?.senderHouse?.trim();
    const floorValue =
      formik.values.senderFloor?.trim() || parcelInfo?.senderFloor?.trim();

    if (!streetValue || !houseValue || !floorValue) {
      toast.error(
        "Please fill in Street, House Number, and Floor before placing the order.",
        {
          position: "bottom-right",
        }
      );
